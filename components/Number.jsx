...
export default function Number({ name: baseName, ...props }) {
  const {
    register,
    formState: { errors },
  } = useFormContext();
  ...
  const name = f + baseName.replaceAll(' ', '_').toLowerCase();
  ...
  return (
    <div className='flex flex-grow flex-col gap-1'>
      <div className='flex gap-1'>
        ...
        <input type='tel' {...props} className='w-full p-2 rounded-md' {...register(name, { required: true })} />
      </div>
      <span>{baseName}</span>
